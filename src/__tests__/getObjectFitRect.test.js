...
describe('getObjectFitRect', () => {
  describe('default alignment', () => {
    ...
    test('calculates image having less width', () => {
      ...
      const { absolute, relative } = getObjectFitRect({ intrinsicSize, renderedSize });
      expect(absolute).toEqual({ top: 0, left: 35, bottom: 0, right: 35, width: 30, height: 100 });
      expect(relative).toEqual({ top: 0, left: '35%', height: '100%', width: '30%' });
    });
    test('calculates image having less height', () => {
      const intrinsicSize = { width: 100, height: 100 };
      const renderedSize = { width: 50, height: 20 };
      const { absolute, relative } = getObjectFitRect({ intrinsicSize, renderedSize });
      expect(absolute).toEqual({ top: 30, left: 0, bottom: 30, right: 0, width: 100, height: 40 });
      expect(relative).toEqual({ top: '30%', left: 0, height: '40%', width: '100%' });
    });
  });
  describe('custom alignment', () => {
    test('calculates image having less width', () => {
      ...
      expect(absolute).toEqual({ top: 0, left: 10.5, bottom: 0, right: 59.5, width: 30, height: 100 });
      expect(relative).toEqual({ top: 0, left: '10.5%', height: '100%', width: '30%' });
    });
    test('calculates image having less height', () => {
      const intrinsicSize = { width: 100, height: 100 };
      const renderedSize = { width: 50, height: 20 };
      ...
      const { absolute, relative } = getObjectFitRect({ intrinsicSize, renderedSize, alignment });
      expect(absolute).toEqual({ top: 48, left: 0, bottom: 12, right: 0, width: 100, height: 40 });
      expect(relative).toEqual({ top: '48%', left: 0, height: '40%', width: '100%' });
    });
  });
});
